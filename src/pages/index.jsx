  ...
  return (
    <div>
      ...
      <div className="flex gap-4 mt-5 flex-wrap ">
        ...
        <button
          onClick={() => sortBy("user", "asc")}
          className="bg-yellow-500 hover:bg-yellow-700 text-white  py-2 px-2 rounded"
        >
           Users (Low to High)
        </button>
        <button
          onClick={() => sortBy("user", "desc")}
          className="bg-yellow-500 hover:bg-yellow-700 text-white  py-2 px-2 rounded"
        >
           Users (High to Low)
        </button>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200 ">
            {data?.map((item, index) => {
              return <TableBody data={item} key={index} index={index + 1} />;
            })}
          </tbody>
        </table>
      </div>
